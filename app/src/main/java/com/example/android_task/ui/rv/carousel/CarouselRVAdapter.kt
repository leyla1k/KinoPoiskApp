...
class CarouselRVAdapter(private val carouselDataList: MutableList<Poster>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CarouselItemViewHolder {
        itemCarouselBinding = ItemCarouselBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: CarouselItemViewHolder, position: Int) {
        ...
        // Загрузка изображения с помощью библиотеки Picasso
            // Picasso.get().load(imageUrl?.url).into(itemCarouselBinding.ivPoster)

        Picasso.get().load(imageUrl?.url).fit().centerInside().error(R.drawable.card_carousel_error).into(itemCarouselBinding.ivPoster)
